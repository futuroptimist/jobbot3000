...
endpoint (rank 6) docks remain intentionally eligible for reordering), and its historical scope
note for why this still didn't reach the two extreme fan-in fixtures.
...
At that point, their `clearanceMargin === -1` evidence was interpreted as a fixed-corridor
signature. [PR #1173](https://github.com/futuroptimist/jobbot3000/pull/1173) later established that
the sentinel conflates fixed-geometry and corridor-bound rejection classes, so it cannot establish
that `RANK_CORRIDOR_HALF_WIDTH` binds the primary sweep. The conclusion that ordering alone did not
fix these fixtures remains valid; the older fixed-width causal claim does not.
...
Neither fixture's underlying infeasibility is fixed. The historical investigation attributed this
to corridor width; the later PR #1173 diagnostic refinement does not support that causal conclusion.
What changed is how the test suite handles it: both tests' actual _contracts_
...
succeed would require a constructive joint route-and-handle placement design that accounts for all
rejection and final-acceptance constraints, not a presumed corridor-width adjustment. Retrying with
a larger `HANDLE_CLEARANCE_TOLERANCE` value alone will not help; the evidence above rules that out
directly.
...
<a id="outstanding-follow-up-work-as-of-this-writing"></a>

## Outstanding follow-up work (authoritative closure status)

The lifecycle test-recovery project is complete. [PR #1173](https://github.com/futuroptimist/jobbot3000/pull/1173)
separated the diagnostic evidence, [PR #1176](https://github.com/futuroptimist/jobbot3000/pull/1176)
shipped P8's immutable per-hop horizontal-geometry seam without changing behavior, and
[PR #1183](https://github.com/futuroptimist/jobbot3000/pull/1183) completed P9 with a gated negative
result. Zero Vitest and zero Playwright tests remain skipped, ordinary supported fixtures remain
green, and the extreme fixtures retain their bounded typed failures.
...
or unfinished test debt. The investigations retained later in this section are historical evidence;
they do not supersede this P8/P9 status.
...
P8 does not calculate density demand, alter rank spacing, change candidate samples or budgets, or
make either extreme fixture feasible. P9 subsequently tested the activation gate and produced the
gated negative result below: no bounded monotone topology-to-spacing rule was established, so
production remains on baseline geometry.
...
Production activation remains gated off. In particular, P9 found that the available topology counts
do not provide a bounded, monotone mapping to spacing that constructively accounts for all three distinct
...
candidate sample, solver budget, or audit exemption is used. If real data justifies reopening this
as a new design project, its acceptance case would need a finite topology-demand domain, a
deterministic maximum SVG width, complete success for both stress fixtures, and parity for baseline
fixtures before changing the production default.

The numbered material below is a historical follow-up list preserved for its investigation record.
Its former open recommendations are closed or superseded by the authoritative status above; it is
not a current implementation backlog.

1. **Make the base D3-Sankey layout `rankOrder`-aware** (a.k.a. "Option 2" above) — **historical;
   the scoped ordering work shipped, and this is not a current recommendation.** Four attempts
   ...
   **Historical pre-PR #1173 interpretation:** this did not close what was then called the
   corridor-width/constructive-placement gap for the two genuinely-infeasible
   ...
   and was not expected to. The retained `clearanceMargin === -1` sentinel was then treated as proof
   of a `RANK_CORRIDOR_HALF_WIDTH` bound. PR #1173 later showed that the sentinel conflates
   fixed-geometry and corridor-bound rejection classes, while the primary-sweep evidence is entirely
   nonincident-route clearance; fallback-only corridor samples do not establish primary causality.
   Both fixtures' tests still characterize their bounded typed failures. This ordering work is also
   not accurately described as the fix
   ...
   (shipped)" below — a different, already-shipped mechanism, not this item.) There is no remaining
   implementation item here: widening the rank corridor, changing order, or increasing budgets is
   not an established fix. Any conditional future research is governed by the closure status above.
...
[Outstanding follow-up work](#outstanding-follow-up-work-as-of-this-writing) for the current status
of the two different, genuinely still-infeasible extreme fixtures. This historical section's
approach is not a current recommendation for them.
...
**Historical conclusion at the time:** this investigation proposed a constructive/greedy strategy
that derived routing-node lane order _jointly_ across all ranks a branch's geometry touched
(effectively [Option 2](#deferred-making-the-base-d3-sankey-layout-rankorder-aware)), rather than a
scoped patch to port spacing, a clearance exemption, or a larger budget. The subsequent P8/P9 work
did not establish that proposal as a fix or an active recommendation. This checked-in test's fixed
evidence (14 blocked branch IDs, 5×55=275 routing-only nodes, ~59.251px feasible lane spacing) remains
useful only as historical input to any new design project governed by the authoritative closure
status above.
