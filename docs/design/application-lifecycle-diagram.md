...
## P6-F3 routed display branches

P6-F3 makes the render layer, not the P4 projection, responsible for visual routing. The projection remains the persisted and exported source of truth for counts, paths, warnings, timeline replay, and taxonomy. Rendering first partitions every aggregate projection link into endpoint-conditioned display branches using `branch:${semanticLinkId}:endpoint:${endpointId}`, then inserts private `route:*` pass-through nodes at every skipped semantic rank. These routing nodes are hidden, non-selectable, omitted from tables and accessibility output, and never written to storage or import/export data.

All visible ribbons are drawn as compound branches made from adjacent-rank segments. Rank centers use the fixed P6-F3 geometry: `SANKEY_NODE_WIDTH = 18`, `LAYOUT_LEFT_MARGIN = 100`, `LAYOUT_RIGHT_MARGIN = 100`, `RANK_CORRIDOR_HALF_WIDTH = 100`, `MINIMUM_TRANSITION_WIDTH = 72`, and `MINIMUM_RANK_CENTER_SPACING = 272`, producing the intentional `1850px` minimum SVG width. Mobile keeps horizontal scrolling local to `.diagram-scroll`; the page keeps natural vertical scrolling.

P6-F3 supersedes P6-F2's visible-node-only height calculation. Routed height uses visible semantic nodes plus hidden routing lanes per rank with `MINIMUM_SVG_HEIGHT = 360`, `LAYOUT_TOP_MARGIN = 64`, `LAYOUT_BOTTOM_MARGIN = 48`, `ROUTED_NODE_PADDING = 72`, and `PER_LANE_VERTICAL_BUDGET = 36`. Adding applications to an existing branch thickens that branch but does not add lanes.

Protected rank corridors reserve each rank center ±100px. Branches remain horizontal inside source and target corridors; all curved vertical interpolation happens only in the empty transition corridor between adjacent rank corridors. The only permitted contact exception is a shared semantic-node docking boundary where flows join or split. Ribbon/node, ribbon/hit-target, and ribbon/label occlusion are prohibited, and labels are centered above visible semantic nodes using deterministic greedy SVG `tspan` wrapping with `NODE_LABEL_MAX_WIDTH = 176` and `NODE_LABEL_MAX_CHARACTERS_PER_LINE = 22`.

Endpoint-conditioned branch colors are stable and supplemental: awaiting response `#60A5FA`, interviewing `#C084FC`, assessment in progress `#FACC15`, offer/negotiating `#2DD4BF`, employer rejected `#FB7185`, candidate withdrew `#FB923C`, offer declined `#F472B6`, offer expired/rescinded `#A3E635`, offer accepted `#4ADE80`, closed/archived `#94A3B8`, and unknown `#E2E8F0`. Branches render with a dark separator, endpoint-colored stroke at 0.82 opacity, and a white selected halo. Each branch has one 44×44 transparent handle in a transition corridor; keyboard access remains through the semantic Flows table.
