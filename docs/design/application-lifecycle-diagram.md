...
## P6-F3 render-only routed branches

P6-F3 makes the render graph distinct from the P4 lifecycle projection. The P4 projection remains authoritative for replay, snapshots, warnings, taxonomy totals, import/export, and persistence; hidden routing nodes are never written to IndexedDB, exported, migrated, or surfaced as lifecycle data.

For rendering, each aggregate projection link is partitioned into endpoint-conditioned display branches keyed as `branch:${semanticLinkId}:endpoint:${endpointId}`. Branches are sorted by canonical endpoint order, source rank, source taxonomy ID, target rank, target taxonomy ID, and semantic link ID. Every branch uses the terminal outcome color: awaiting response `#60A5FA`, interviewing `#C084FC`, assessment in progress `#FACC15`, offer/negotiating `#2DD4BF`, employer rejected `#FB7185`, candidate withdrew `#FB923C`, offer declined `#F472B6`, offer expired/rescinded `#A3E635`, offer accepted `#4ADE80`, closed/archived `#94A3B8`, and unknown `#E2E8F0`.

Rank-skipping branches receive private zero-width pass-through routing nodes at every skipped semantic rank. D3 Sankey lays out this expanded render-only graph, then all horizontal positions are overwritten to fixed rank centers. Rendered branch segments are always adjacent-rank; `sankeyLinkHorizontal()` is not used for final visible branch geometry.

The normative constants are: node width `18`, minimum SVG height `360`, top margin `64`, bottom margin `48`, routed node padding `72`, per-lane vertical budget `36`, maximum label width `176`, label wrap width `22` characters, rank corridor half-width `100`, minimum transition width `72`, left margin `100`, right margin `100`, and minimum rank-center spacing `272`. The minimum routed SVG width is `1850`, so mobile keeps the diagram-local horizontal scroller rather than shrinking the SVG.

Each rank reserves a protected corridor from `rankCenterX - 100` through `rankCenterX + 100`. Links remain horizontal inside source and target rank corridors. All vertical interpolation occurs only in the empty transition corridor between adjacent protected corridors. The only allowed contact exception is a shared semantic-node docking boundary where flows join or split. Ribbons, dark separators, selection halos, and 44px handles must not occlude unrelated visible nodes, labels, or hit rectangles.

Node labels are centered above their semantic node, greedily wrapped into SVG `tspan` lines without browser text measurement, and are never placed inside incoming or outgoing ribbon bands. P6-F3 supersedes P6-F2's visible-node-only density calculation: height is based on the densest rank counting both active visible semantic nodes and active private routing nodes. Adding applications to an existing aggregate branch may thicken a ribbon but must not add routing lanes or increase height; adding a new endpoint-conditioned branch may do both.

Branch rendering uses three deterministic layers: optional white selected halo, dark separator, and endpoint-colored visible branch at opacity `0.82` (or `1` when selected). Selected nodes retain their base color and add a white outline instead of relying on amber. Each branch receives one transparent 44px handle in a transition corridor. The semantic Flows table uses the same endpoint-conditioned branch model as the SVG.
