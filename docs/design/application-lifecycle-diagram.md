...
For the deterministic lane/handle-placement solver's internals, see [lifecycle-diagram-layout-algorithm.md](./lifecycle-diagram-layout-algorithm.md). Current route auditing rejects unsafe layouts, and the reference routing fixture is crossing-safe; the deferred limitation is making the D3-Sankey base layout `rankOrder`-aware for known-infeasible dense fixtures, which currently fail safely through the existing fallback.
...
Each rank reserves a protected horizontal corridor from `rankCenterX - 100` through `rankCenterX + 100`. Visible semantic nodes are centered in that corridor and routing nodes sit at the same rank center with zero visible width. Branch paths do not use full-length `sankeyLinkHorizontal()` curves. Instead, each adjacent-rank segment stays horizontal inside the source and target protected corridors; all vertical interpolation happens only in the empty transition corridor between adjacent ranks. The transition corridor is always at least `72px` wide.

Labels are centered over visible semantic nodes, placed above the node rather than inside incoming or outgoing ribbons, and wrapped deterministically into at most two SVG `tspan` lines without truncating semantic text. Label layout uses fixed taxonomy text and the constants above, not browser font measurements, `getBBox()`, canvas measurement, or resize-observer-derived geometry.
