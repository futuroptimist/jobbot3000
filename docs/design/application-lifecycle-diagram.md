...
The only permitted contact between branch geometry and semantic nodes is the shared semantic-node docking boundary where flows join or split. Otherwise ribbons, dark separators, selection halos, and 44px branch handles must not intersect unrelated node rectangles, labels, or hit rectangles. Routing lanes must not pass behind intermediate milestones, and unrelated branches must not share coincident centerline runs. Repository fixtures are expected to have zero unrelated branch crossings; if a future fixed-order topology makes an isolated crossing unavoidable, it may occur only inside an empty transition corridor and never over nodes or labels.
