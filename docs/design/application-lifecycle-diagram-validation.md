...
| Historical timeline, unknown-date, date-only, exact-time, inclusive cutoff, simultaneous buckets | P4 unit tests plus `test/playwright/lifecycle-diagram.spec.js` seeded journey, including both no-notice ordinary historical navigation and the positive newer-activity notice after a supported lifecycle mutation while history remains selected |
...
P6-F1 adds deterministic browser coverage for both sides of the historical-update contract: ordinary historical navigation, scrubbing, and leaving/returning to Diagram do not show `Newer activity available` when effective lifecycle data is unchanged; a supported application-form status change made while a historical bucket remains selected does show the notice through the existing polite live region without moving the range from that historical bucket or leaking the newer event into the historical projection.
