...
## P6-F2 density-aware spacing validation

P6-F2 replaces the previous coarse `70 + projection.nodes.length * 34` height estimate and `16px` node padding with deterministic density-aware sizing. The implementation is validated at three levels:

| Spacing requirement                                                                                                                | Vitest/component coverage                                                                                                              | Performance coverage                                                                                                                 | Playwright coverage                                                                                                                    |
| ---------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------- |
| Helper returns exact contractual heights for busiest-rank counts 1, 3, 5, 10, and 11                                               | `test/web-tracker-lifecycle-diagram.test.js` checks `calculateLifecycleDiagramLayout` results of 360, 360, 420, 820, and 900px         | Same helper used to compare small and 1,000-application projections                                                                  | Imported fixture SVG height is compared with the deterministic formula                                                                 |
| Invalid widths fall back to 760px and desktop widths above 760px are preserved                                                     | Focused helper tests cover `undefined`, zero, negative, `NaN`, `Infinity`, and `1200.9`                                                | Large render uses a 1200px container while preserving density-derived height                                                         | Desktop and 375×812 mobile contexts assert SVG/viewBox geometry                                                                        |
| Height depends only on active aggregate-node density, not input order, application count, event count, or zero-count taxonomy rows | Helper tests cover shuffled nodes, zero-total nodes, non-busiest-rank additions, busiest-rank additions, and deeply frozen projections | 10-app and 1,000-app bundles with the same aggregate taxonomy density produce the same calculated height; render stays under 5,000ms | Imported fixture checks assert bounded aggregate SVG geometry rather than per-application nodes                                        |
| SVG height, viewBox, Sankey extent, and node padding consistently use the calculated layout                                        | Component tests assert sparse 360px render, dense fixture 900px render, and matching `viewBox` height                                  | Large render verifies the DOM SVG height equals the helper result                                                                    | Playwright checks SVG height/viewBox agreement in desktop and mobile contexts                                                          |
| Adjacent visible node rectangles in each rank have at least 44px vertical separation with 0.5px tolerance                          | Component geometry assertions group visible rectangles by horizontal rank and check gaps                                               | Large-data SVG coordinates are checked for `NaN`/`Infinity`                                                                          | Playwright DOM/SVG geometry groups visible node rectangles by rank and checks non-overlap and minimum gap                              |
| Transparent 44px node hit regions in a rank do not overlap                                                                         | Component tests group `[data-diagram-node-hit]` rectangles by rank and assert boundaries do not overlap                                | Large render confirms no per-application hit nodes are introduced                                                                    | Playwright checks transparent hit-region non-overlap and touch selection                                                               |
| Node rectangles, labels, hit regions, and first/last rank nodes remain within SVG vertical bounds                                  | Component tests assert top and bottom visible node bounds against the 32px margins                                                     | Finite coordinate/path assertions cover large aggregate renders                                                                      | Playwright checks node bounds, label clipping, scroll-container height, and page overflow                                              |
...
## P6-F3 requirement-to-test matrix

| Requirement                                                             | Validation                                                                                                                                                                                                   |
| ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Endpoint-conditioned display branches and hidden routing-node expansion | `test/web-tracker-lifecycle-diagram-layout.test.js` covers partitioning, deterministic IDs/order, skipped-rank routing nodes, adjacent-rank segments, and routing-node exclusion from semantic nodes.        |
| Protected-corridor collision geometry                                   | Component and Playwright lifecycle-diagram checks inspect finite adjacent-rank geometry, node/label protected placement, handles, no page overflow, and no force-click bypass.                               |
| Branch colors and contrast                                              | Layout unit tests assert the exact endpoint palette and unknown fallback; component/Playwright checks assert endpoint-colored branches and selected branches retain their base color.                        |
| Semantic-table parity                                                   | Component tests compare SVG branch IDs with Flows table rows and verify selection/details use display branches rather than unsplit projection links.                                                         |
| Touch handles                                                           | Layout tests assert one deterministic non-overlapping 44×44 handle per branch; Playwright verifies touch and click selection.                                                                                |
| Stress fixture                                                          | `test/fixtures/tracker-lifecycle-diagram-routing-v2.json` contains 25 synthetic applications reproducing the long awaiting-response branch plus interviewing, rejected, and assessment-in-progress outcomes. |
| Desktop/mobile                                                          | Playwright covers desktop and 375×812 mobile current and historical diagram views with local `.diagram-scroll` horizontal scrolling.                                                                         |
| Performance bounds                                                      | `test/web-tracker-lifecycle-diagram-performance.test.js` keeps the existing 5,000ms limit and verifies routes depend on aggregate branch structure rather than application count.                            |
| Actions-only visual artifacts                                           | `.github/workflows/diagram-visual-review.yml` captures four PNG artifacts from runner temporary storage only and includes the routing fixture in its path filter.                                            |
