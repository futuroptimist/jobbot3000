...
| Historical live-update contract                                                                  | `test/playwright/lifecycle-diagram.spec.js` covers both sides: ordinary historical navigation shows no newer-activity notice, while a supported application-form lifecycle mutation announces `Newer activity available` without leaving the selected historical snapshot |
...
- Playwright coverage now exercises both sides of that historical-update contract: entering/navigating existing history and returning from Applications without lifecycle changes keep the notice absent, while changing `Synthetic app-01` from `applied` to `technical_screen` through the supported application form under a fixed browser time announces `Newer activity available` exactly once, preserves the selected historical `aria-valuetext`, and keeps the historical table totals unchanged until the user returns to Current.
