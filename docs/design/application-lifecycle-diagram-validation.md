...
| Historical timeline, unknown-date, date-only, exact-time, inclusive cutoff, simultaneous buckets | P4 unit tests plus `test/playwright/lifecycle-diagram.spec.js` seeded and mutation journeys                                          |
...
- Playwright now covers both sides of the historical-update contract: ordinary historical navigation, scrubbing among existing buckets, and navigation away/back do not show `Newer activity available`; a supported application status mutation while a historical bucket remains selected does show the notice while preserving the selected historical snapshot and projection totals.
