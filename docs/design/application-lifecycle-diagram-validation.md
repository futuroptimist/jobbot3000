...
| Spacing/layout requirement                                                                                                                      | Vitest coverage                                                                                                                                                                                                         | Performance coverage                                                                                                                             | Playwright coverage                                                                                                                                           |
| ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Helper uses active aggregate nodes grouped by busiest rank, ignores zero-total nodes, preserves width fallback, and does not mutate projections | `test/web-tracker-lifecycle-diagram.test.js` covers exact 1/3/5/10/11-node heights, invalid widths, shuffled nodes, non-busiest additions, busiest-rank growth by `36 + 44`, zero totals, and deeply frozen projections | `test/web-tracker-lifecycle-diagram-performance.test.js` compares small and 1,000-application projections with the same bounded taxonomy density | `test/playwright/lifecycle-diagram.spec.js` validates imported fixture SVG height against the density formula on desktop and 375×812 touch mobile             |
| SVG `height`, `viewBox`, D3 extent, node padding, and natural scroll container height use the calculated canvas height                          | `test/web-tracker-lifecycle-diagram.test.js` asserts sparse 360px rendering, dense 900px fixture rendering, and matching SVG/viewBox heights                                                                            | Large-data render asserts calculated height is unchanged by application volume                                                                   | Desktop and mobile geometry checks assert matching SVG/viewBox heights and that `.diagram-scroll` does not clip the SVG bottom                                |
| Adjacent visible nodes in each rank retain at least 44px of separation and transparent 44px node hit regions do not overlap                     | `test/web-tracker-lifecycle-diagram.test.js` groups rendered SVG rectangles by rank and checks visible-node gaps plus hit-region non-overlap with 0.5px tolerance                                                       | Large-data render ensures no per-application/company nodes are introduced and aggregate nodes remain bounded at 21                               | Playwright groups DOM/SVG geometry by horizontal rank on desktop and mobile to check visible nodes and hit regions do not overlap                             |
| Node rectangles, labels, hit regions, link widths, and path data remain finite and inside vertical SVG bounds                                   | `test/web-tracker-lifecycle-diagram.test.js` checks finite rectangle coordinates and top/bottom margins inside the SVG                                                                                                  | `test/web-tracker-lifecycle-diagram-performance.test.js` rejects `NaN`/`Infinity` in SVG geometry/path output under 1,000-application input      | Playwright checks first/last visible nodes and labels stay within the SVG and records no console/page errors                                                  |
| Responsive behavior keeps page-level horizontal overflow absent while mobile overflow remains local to `.diagram-scroll`                        | Existing Vitest selection/rerender coverage preserves calculated height after selection updates                                                                                                                         | Large-data test stays under the existing 5,000ms limit without volume-based height growth                                                        | Mobile Playwright verifies no page overflow, diagram-local horizontal overflow, touch node/flow selection, semantic-table keyboard parity, and axe compliance |
