...
function exportData(fmt, options = {}) {
  const b = state.bundle;
  if (fmt === "json")
    download(
    ...
    download(
      options.filename || "jobbot3000-applications.csv",
      "text/csv",
      csv([head, ...rows]),
    );
  }
...
function init() {
  ...
  const settingsMessage = $("[data-settings-message]");
  $("[data-backup-now]").onclick = async () => {
    exportData("json", { filename: `jobbot3000-backup-${day(now())}.json` });
    settingsMessage.textContent = "Backup download prepared in this browser.";
  };
  $("[data-clear-data]").onclick = async () => {
    if (
      confirm(
        "Clear all local tracker data from this browser? This cannot be undone unless you have a backup export.",
      )
    ) {
      await repo.clear();
      settingsMessage.textContent =
        "Local tracker data cleared from IndexedDB.";
      refresh();
    }
  };
  refresh();
