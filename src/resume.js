...
function collectAmbiguities(text) {
  if (!text) return [];
  const ambiguities = [];
  const seen = new Set();
  const lineStarts = createLineStartIndex(text);

  const addFinding = (type, matchValue, message, matchIndex) => {
    const value = matchValue.trim();
    if (!value) return;
    const key = `${type}:${value.toLowerCase()}`;
    if (seen.has(key)) return;
    seen.add(key);
    const location = findLocation(lineStarts, matchIndex);
    ambiguities.push({ type, value, message, location });
  };

  for (const match of collectMatches(DATE_PLACEHOLDER_PATTERNS, text)) {
    addFinding('date', match.value, 'Potential placeholder date detected', match.index);
  }

  for (const match of collectMatches([TITLE_PLACEHOLDER_PATTERN], text)) {
    addFinding('title', match.value, 'Potential placeholder title detected', match.index);
  }

  for (const match of collectMatches(METRIC_PLACEHOLDER_PATTERNS, text)) {
    if (/^\+?$/.test(match.value.trim())) continue;
    addFinding('metric', match.value, 'Potential placeholder metric detected', match.index);
  }
...
export async function loadResume(filePath, options = {}) {
  ...
  const ambiguities = collectAmbiguities(text);
