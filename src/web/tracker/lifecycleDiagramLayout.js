...
export function createLifecycleHorizontalGeometry({
  ...
  const baselineWidth =
    ...
    rightMargin +
    nodeWidth +
    (ranks.length - 1) * MINIMUM_RANK_CENTER_SPACING;
  const leftOuterExtent = centers[0] - nodeWidth / 2;
  const rightOuterExtent = centers[ranks.at(-1)] + nodeWidth / 2;
  const svgWidth =
  ...
  const hops = {};
  for (let sourceRank = 0; sourceRank < ranks.length - 1; sourceRank += 1) {
...
export function calculateLifecycleDiagramLayout(
  ...
  const maximumActiveRank = Math.max(
    6,
    ...graph.nodes.map((node) => node.rank),
  );
  horizontalGeometry ??=
    maximumActiveRank === 6
      ? BASELINE_LIFECYCLE_HORIZONTAL_GEOMETRY
      : createLifecycleHorizontalGeometry({
          rankCount: maximumActiveRank + 1,
        });
  ...
  for (const link of graph.links ?? []) {
    ...
    if (sourceRank < 0 || sourceRank >= maximumActiveRank) {
      throw new Error(
        [
          "Lifecycle diagram layout invariant violated:",
          `link ${link.id ?? "<unknown>"}`,
  ...
  return {
    ...
    maximumActiveRank,
    rankCount: maximumActiveRank + 1,
    transitionCount: maximumActiveRank,
  };
}
...
function layoutLifecycleRoutingGraphPass(
  ...
  let horizontalGeometry = options.horizontalGeometry;
  ...
  const rankCount = Math.max(7, ...graph.nodes.map((node) => node.rank + 1));
  horizontalGeometry ??=
    rankCount === 7
      ? BASELINE_LIFECYCLE_HORIZONTAL_GEOMETRY
      : createLifecycleHorizontalGeometry({ rankCount });
...
export function buildLifecycleRouteModel(graph, dimensions) {
  ...
  const transitionCount = Math.max(6, ...graph.nodes.map((node) => node.rank));
  ...
  for (const link of graph.links ?? []) {
    ...
    if (link.source?.rank >= 0 && link.source.rank < transitionCount)
