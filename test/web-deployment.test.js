...
describe("web deployment artifacts", () => {
  ...
  it("builds a static browser-only tracker with health probes and security headers", async () => {
    const packageJson = JSON.parse(
      await readFile(path.join(repoRoot, "package.json"), "utf8"),
    );
    expect(packageJson.scripts.build).toBe("node scripts/build-web-static.js");

    const buildScript = await readFile(
      path.join(repoRoot, "scripts", "build-web-static.js"),
      "utf8",
    );
    expect(buildScript).toContain("web-dist");
    expect(buildScript).toContain("healthz");
    expect(buildScript).toContain("livez");
    expect(buildScript).toContain("Content-Security-Policy");
    expect(buildScript).not.toMatch(
      new RegExp(
        "better-sqlite3|opportunities\\.db|appendFile|" +
          "writeFile\\([^\\n]*(application|outreach|notes|csv)",
        "i",
      ),
    );
  });
