...
describe("web server command endpoint", () => {
  ...
  it("logs invalid payload attempts into per-client history", async () => {
    const commandAdapter = {
      summarize: vi.fn(async () => ({ ok: true })),
    };
    ...
    const response = await fetch(`${server.url}/commands/summarize`, {
      ...
      body: JSON.stringify({ input: 123, token: "supersecret" }),
    });
    ...
    expect(body.entries).toEqual([
      {
        command: "summarize",
        payload: { input: 123, token: "***redacted***" },
        result: {
          status: "error",
          error: expect.stringMatching(/input|payload/i),
        },
        timestamp: expect.any(String),
      },
    ]);
  ...
  it("preserves adapter status while annotating payload history results", async () => {
    const commandAdapter = {
      summarize: vi.fn(async () => ({
        status: "processing",
        detail: "queued",
    ...
    expect(response.status).toBe(200);
    const body = await response.json();
    expect(body).toEqual({ status: "processing", detail: "queued" });
  ...
  it("rate limits invalid content type requests", async () => {
    const commandAdapter = {
      summarize: vi.fn(async () => ({ ok: true })),
    };
