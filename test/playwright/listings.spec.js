...
test.describe('Listings tab', () => {
  ...
  test('fetches and ingests a listing using the default All provider', async ({ page }) => {
    ...
    await page.locator('[data-listings-filter="title"]').fill('Software Engineer');
    await page.locator('[data-listings-submit]').click();

    await expect(page.locator('.listing-card')).toHaveCount(2);
    await expect(page.locator('.listing-card').first()).toContainText('Staff Software Engineer');
    ...
    await expect(page.locator('.listing-card').first().locator('.listing-card__badge')).toHaveText(
      'Ingested',
    );
    await expect(page.locator('[data-listings-message]')).toContainText('Listing ingested');

    await expect.poll(() => ingestCalls.length).toBe(1);
    expect(ingestCalls[0]).toMatchObject({ provider: 'greenhouse', identifier: 'acme-co' });
  });
});
