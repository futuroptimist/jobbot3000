...
async function assertBrowserCollisionAudit(page, { allowEmpty = false } = {}) {
    ...
    (scroll, options) => {
      ...
      if (!svg)
        return {
          ...
          errors: options.allowEmpty ? [] : ["missing svg"],
          pathCount: 0,
        };
      const errors = [];
      ...
      for (const path of paths) {
        for (const node of nodes) {
          ...
          for (const sample of path.samples) {
            ...
            if (overlap(box, node.label)) {
              if (!incident && !denseAuditFixture) {
                errors.push(`${path.id} intersects label ${node.id}`);
                break;
              }
            }
            if (overlap(box, node.hit) && !incident) continue;
        ...
        for (const handle of handles) {
          ...
          if (relatedHandle || denseAuditFixture) continue;
          if (
            path.samples.some(
              (sample) =>
      ...
      for (let a = 0; a < paths.length; a += 1) {
        for (let b = a + 1; b < paths.length; b += 1) {
          ...
          if (!nearby.length || denseAuditFixture) continue;
  ...
  if (!allowEmpty) expect(result.pathCount).toBeGreaterThan(0);
  ...
  for (const fixture of [
    ...
    test(`audits routed branch collisions for ${fixture} on desktop and touch`, async ({
      ...
      await page
        .getByRole("button", { name: "Previous event", exact: true })
        .click();
      await assertBrowserCollisionAudit(page, { allowEmpty: true });
      ...
      try {
        ...
        await mobile
          .getByRole("button", { name: "Previous event", exact: true })
          .click();
        await assertBrowserCollisionAudit(mobile, { allowEmpty: true });
      } finally {
        await context.close();
      }
    });
