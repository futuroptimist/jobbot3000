...
async function cmdTrackHistory(args) {
  ...
  if (!events.length) {
    console.log(`No events logged for ${jobId}`);
    return;
  }

  const lines = [`History for ${jobId}:`];
  events.forEach((event, idx) => {
    const channel = event.channel || 'event';
    const when = event.date || 'unknown date';
    lines.push(`${idx + 1}. ${when} — ${channel}`);
...
async function cmdTrack(args) {
  ...
  console.error('Usage: jobbot track <add|log|discard|reminders|history> ...');
  process.exit(2);
}
